...
// lets define the contract of the object (bsically we have to tell which data we are dealing with )
//  in general  we call it intereface 
interface UserData {
    id: number;
    login: string;
    avatar_url: string;
...
formSubmit.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (matchingUsers.length === 0) {
            main_container?.insertAdjacentHTML(
                "beforeend",// it means at the bottom of the element 
